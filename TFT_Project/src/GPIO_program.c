...
void MGPIO_voidSetAlternativeFunction(u8 Copy_u8PortName ,u8 Copy_u8PinNumber , u8 Copy_u8AlterantiveNumber)
{
	switch (Copy_u8PortName)
	{
	case MGPIO_PORTA:
		...
		{
			GPIOA_AFRL &= ~(0b1111 << (Copy_u8PinNumber * 4));
			GPIOA_AFRL |= (Copy_u8AlterantiveNumber << (Copy_u8PinNumber * 4) );
		}
		else if (Copy_u8PinNumber <= 15)
		{
			GPIOA_AFRL &= ~(0b1111 << ((Copy_u8PinNumber-8) * 4));
			GPIOA_AFRL |= (Copy_u8AlterantiveNumber << ((Copy_u8PinNumber-8) * 4) );
		}
		break;
	}
}
